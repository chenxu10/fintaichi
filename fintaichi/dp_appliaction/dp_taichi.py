...
if benchmark:
    ...
    a_array = np.array([i for i in range(51, -1, -1)])
    b_array = np.array([1,2,3,4,5,0])

@ti.kernel
def modifydp(s1: ti.types.ndarray(), s2: ti.types.ndarray()) -> ti.i32:
    n = s1.shape[0]
    m = s2.shape[0]
    ti.loop_config(serialize=True)
    for i in list(range(0,52))[::-1]:
...
print(modifydp(a_array, b_array))
